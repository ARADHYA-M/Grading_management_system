...
def profile():
    ...
    for item in data:
        ...
        if grade == 'U':
            total_credits = 0
            break
        grade_point = grade_convertor.get(grade, 0)
        
        # Assuming each course has 3 credits
        credits = 3
        
        # Add grade points and credits for valid grades
        if grade_point != 0:
            total_credit_points += grade_point * credits
            total_credits += credits

    # Calculate CGPA if total credits are non-zero
    if total_credits != 0:
        cgpa = total_credit_points / total_credits

    print(cgpa)
    cgpa = round(cgpa,2)
    return render_template('profile.html', name=student.name, roll_number=student.roll_number, title=student.name, data=data, failed_data=failed_data,mark=cgpa)
